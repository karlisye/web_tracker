...
const InactiveWebsitesTable = () => {
  ...
  return (
    ...
    <div className='bg-teal-700 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-8'>
      <table className='w-full table-fixed text-left border-separate border-spacing-y-2'>
        <thead>
          <tr className='shadow-sm rounded-md'>
            ...
            <th className='bg-yellow-100 p-2 rounded-r-md'>Last Visit</th>
          </tr>
        </thead>
        <tbody>
          {inactiveWebsites.map(inactiveWebsite => (
            <tr className='shadow-sm rounded-md' key={inactiveWebsite.host}>
              <td className='bg-yellow-100 p-2 rounded-l-md'><a target='_blank' href={`https://${inactiveWebsite.host}`}>{inactiveWebsite.host}</a></td>
              <td className='bg-yellow-100 p-2 rounded-r-md'>
